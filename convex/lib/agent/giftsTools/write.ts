...
export const createGiftEventTool: ToolDefinition = {
  ...
  description:
    "Crear un evento de regalos (cumpleaños, navidad, graduación, etc). Usa esto para organizar una lista de regalos para una ocasión especial.",
  parameters: {
...
export const addGiftToEventTool: ToolDefinition = {
  name: "addGiftToEvent",
  description:
    "Agregar un regalo a un evento existente. El regalo puede estar asignado a un destinatario específico o quedar en un 'pool' general.",
  parameters: {
    type: "object" as const,
    properties: {
      eventName: {
      ...
      recipientName: {
        type: "string" as const,
        description:
          "Nombre de la persona que recibirá el regalo (opcional, si no se especifica queda sin asignar)",
      },
      title: {
        type: "string" as const,
...
export async function handleAddGiftToEvent(context: ToolContext, args: Record<string, unknown>) {
  ...
  if (recipientName) {
    recipientId = await findOrCreateRecipientId(context, event._id, recipientName);
  }
...
export const updateGiftStatusTool: ToolDefinition = {
  ...
  description:
    "Actualizar el estado de un regalo (idea → to_buy → bought → wrapped → delivered). Usa esto para marcar progreso en la compra o entrega de regalos.",
  parameters: {
    type: "object" as const,
    properties: {
      eventName: {
...
export async function handleUpdateGiftStatus(context: ToolContext, args: Record<string, unknown>) {
  ...
  await context.ctx.runMutation(api.gifts.updateGiftItem, {
    ...
    status: newStatus as GiftStatus,
  });
...
export const updateGiftItemTool: ToolDefinition = {
  name: "updateGiftItem",
  description:
    "Actualizar cualquier información de un regalo (título, precio, notas, URL, etc). Más completo que updateGiftStatus.",
  parameters: {
    type: "object" as const,
    properties: {
      eventName: {
...
export async function handleUpdateGiftItem(context: ToolContext, args: Record<string, unknown>) {
  ...
  if (newStatus) updates.status = newStatus as GiftStatus;
